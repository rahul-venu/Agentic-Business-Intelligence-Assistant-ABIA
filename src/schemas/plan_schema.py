...
class JoinSpec(BaseModel):
    table: str = Field(
        ..., description="Table name to join ('customer_data' or 'product_data')"
    )
    on: str = Field(
        ..., description="Key column name to join on ('customer_id' or 'product_id')"
    )
    how: Literal["inner", "left", "right"] = Field(
        default="left", description="Join type"
    )
...
class AggregationSpec(BaseModel):
    ...
    function: Literal["sum", "mean", "count", "nunique", "min", "max"] = Field(
...
class QueryPlan(BaseModel):
    thought_process: str = Field(
        ..., description="Step-by-step logic explaining the query interpretation"
    )
    primary_table: str = Field(
        default="sales_data", description="Base table for query execution"
    )
    ...
    @field_validator(
        "joins", "filters", "group_by", "aggregations", "derived_metrics", mode="before"
    )
    @classmethod
